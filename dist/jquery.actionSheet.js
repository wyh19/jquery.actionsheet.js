...
*数据支持本地(json对象)和远程模式(url)；形式支持文字列表和带图标的九宫格形式
*/
; (function ($) {
    $.fn.ActionSheet = function (options) {
        ...
        function gridRender(mode, data) {
            $.each(data, function (i) {
                $this.append('<div class="as-grid-cell" data-id="' + data[i][settings.idField] + '">' +
        ...
        function gridLayout() {
            ...
            $this.css('bottom', -$this_height + 'px').show();
            $bg.hide();
        }
        //list模式下数据绘制
        function listRender(mode, data) {
            $.each(data, function (i) {
                $this.append('<div class="as-list-cell" data-id="' + data[i][settings.idField] + '">' +
        ...
        function hideActionSheet() {
            $this.animate({ bottom: -$this_height + 'px' }, 300);
            $bg.hide();
        ...
        function bindHandler(mode) {
            ...
            $this.on(settings.clickType, function (e) {
                ...
                //执行业务
                settings.callback.call(null, $tar.data('id'));
                //执行完隐藏
                hideActionSheet();
            });
        }
